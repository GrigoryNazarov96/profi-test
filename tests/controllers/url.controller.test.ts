...
describe("POST /api", () => {
  it("should create a shortened URL with a valid original URL", async () => {
    const response = await _app.post("/api").send({ originalUrl: "https://www.example.com" });
    expect(response.status).toBe(200);
    expect(response.body.shortUrl).toBeDefined();
  });

  it("should handle invalid original URL", async () => {
    const response = await _app.post("/api").send({ originalUrl: "invalid-url" });
  ...
  it("should handle duplicate custom URL", async () => {
    ...
    await _app.post("/api").send({ originalUrl: "https://www.example2.com", customUrl });
    const response = await _app.post("/api").send({ originalUrl: "https://www.example3.com", customUrl });
...
describe("GET /api/:code", () => {
  it("should redirect to the original URL when the short URL exists", async () => {
    const response1 = await _app.post("/api").send({ originalUrl: "https://www.example4.com" });
    const shortUrl = response1.body.shortUrl;
    const response2 = await _app.get(`/api/${shortUrl}`);
    expect(response2.status).toBe(302);
  });

  it("should handle non-existent short URL", async () => {
    const response = await _app.get("/api/non-existent-short-url");
